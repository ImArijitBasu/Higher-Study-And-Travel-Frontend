...
const CarouselCards = () => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      setSlideWidth(containerRef.current.scrollWidth / 2); // since we duplicate slides
    }
  }, []);
      ...
      <div ref={containerRef} className="overflow-hidden">
        ...
        >
          ...
          {[...storiesData, ...storiesData].map((story) => (
            <div key={story.id + Math.random()} className="flex-shrink-0 w-72 h-96 mr-6 relative rounded-lg overflow-hidden shadow-xl">
              <Image
                src={story.imageUrl}
                alt={story.title}
                fill
